!------------------------------------------------------------------------------
! s2dw_fileio_mod  -- S2DW library file IO class
! 
...
module s2dw_fileio_mod
  ...
  public :: &
    s2dw_fileio_matlab_wav_write, &
    s2dw_fileio_matlab_wav_read, &
		s2dw_fileio_fits_wav_write, &
		s2dw_fileio_fits_wav_read
  ...
  interface s2dw_fileio_matlab_wav_write
     module procedure &
       s2dw_fileio_matlab_wav_write_static, &
       s2dw_fileio_matlab_wav_write_dynamic
  end interface

  interface s2dw_fileio_matlab_wav_read
     module procedure &
       s2dw_fileio_matlab_wav_read_static, &
       s2dw_fileio_matlab_wav_read_dynamic
  end interface

  interface s2dw_fileio_fits_wav_write
     module procedure &
       s2dw_fileio_fits_wav_write_static, &
       s2dw_fileio_fits_wav_write_dynamic
  end interface

  interface s2dw_fileio_fits_wav_read
     module procedure &
  ...
  contains
    ...
    subroutine s2dw_fileio_matlab_wav_write_static(wav, scoeff, J, B, N, &
      ...
      write(fileid, '(a,a)') '% function [J, N, B, bl_scoeff, alpha, scoeff, wcoeff] = ', &
           filename(1:len(trim(filename))-2)
      write(fileid, '(a)') '%'
      write(fileid, '(a)') '%  Scale discretised wavelet coefficients created by S2DW code'
      write(fileid, '(a)') '%  S2DW code written by Jason McEwen (mcewen@mrao.cam.ac.uk)'
      write(fileid, '(a)') '%'
    ...
    subroutine s2dw_fileio_matlab_wav_write_dynamic(wavdyn, scoeff, J, B, N, &
      bl_scoeff, alpha, filename, comment)
			...
			integer, intent(in) :: bl_scoeff
			real(dp), intent(in) :: alpha
      type(s2dw_wav_abg), intent(in), allocatable :: wavdyn(:)
			complex(dpc), intent(in) :: scoeff(0:bl_scoeff-1, 0:bl_scoeff-1)
      character(len=*), intent(in) :: filename
      character(len=*), intent(in), optional :: comment
    ...
    !!     S2DW Fortran or Matlab routine (file error checking is *not* 
    !!     performed).
    !!
    !! Variables:
    ...
    subroutine s2dw_fileio_matlab_wav_read_static(wav, scoeff, J, B, N, &
      ...
      fail = 0
			...
			if(fail /= 0) then
				call s2dw_error(S2DW_ERROR_MEM_ALLOC_FAIL, 's2dw_fileio_matlab_wav_read_static')
			end if
    ...
    subroutine s2dw_fileio_matlab_wav_read_dynamic(wavdyn, scoeff, J, B, N, &
         bl_scoeff, alpha, filename)
      ...
      allocate(wavdyn(0:J), stat=fail)
			allocate(scoeff(0:bl_scoeff-1, 0:bl_scoeff-1), stat=fail)
			if(fail /= 0) then
				call s2dw_error(S2DW_ERROR_MEM_ALLOC_FAIL, 's2dw_fileio_matlab_wav_read_dynamic')
			end if
			scoeff(0:bl_scoeff-1, 0:bl_scoeff-1) = 0d0
    ...
    subroutine s2dw_fileio_fits_wav_write_static(wav, scoeff, J, B, N, &
         bl_scoeff, alpha, filename, comment)
      ...
      call s2dw_fileio_fits_exists(filename, status, file_exists)
      if(file_exists) then
         call s2dw_error(S2DW_ERROR_FILEIO, &
						's2dw_fileio_fits_wav_write', &
      ...
      call ftpcom(unit, &
        '  Scale discretised wavelet coefficients created by S2DW code',status)
      call ftpcom(unit, &
      ...
      ! Check for errors.
      if (status .gt. 0) call s2dw_fileio_fits_error_check(status, .true.)
    ...
    subroutine s2dw_fileio_fits_wav_write_dynamic(wavdyn, scoeff, J, B, N, &
      bl_scoeff, alpha, filename, comment)
    ...
    subroutine s2dw_fileio_fits_wav_read_static(wav, scoeff, J, B, N, &
         bl_scoeff, alpha, filename)
      ...
      fail = 0
			...
			if(fail /= 0) then
				call s2dw_error(S2DW_ERROR_MEM_ALLOC_FAIL, 's2dw_fileio_fits_wav_read')
			end if
			wav(0:J, 0:2*B-2, 0:2*B-1, 0:N-1) = 0d0
			scoeff(0:bl_scoeff-1, 0:bl_scoeff-1) = 0d0
    ...
    subroutine s2dw_fileio_fits_wav_read_dynamic(wavdyn, scoeff, J, B, N, &
         bl_scoeff, alpha, filename)
    ...
    subroutine s2dw_fileio_fits_error_check(status, halt)
    ...
    subroutine s2dw_fileio_fits_exists(filename, status, exists)
      ...
      if(status == 0) then
        ...
        halt = .false.
        call s2dw_fileio_fits_error_check(status, halt)
        call ftclos(unit, status)
        status = 0
        exists = .true.
    ...
    subroutine s2dw_fileio_fits_del(filename, status)
