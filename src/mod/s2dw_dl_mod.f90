...
!!
!! Note:
!!  - Copied from s2 library so S2DW library is self contained.
!
!! @author D. J. Mortlock
...
contains
  ...
  subroutine s2dw_dl_beta_operator(dl, beta, l)
    ...
    call s2dw_dl_beta_operator_core(dl(-l:l,-l:l), beta, l)

    ! Use its symmetry properties to fill in the rest of it.
    call s2dw_dl_beta_operator_fill(dl(-l:l,-l:l), l)
  ...
  subroutine s2dw_dl_beta_operator_core(dl, beta, l)
    integer, intent(in) :: l
  ...
  subroutine s2dw_dl_beta_operator_fill(dl, l)
