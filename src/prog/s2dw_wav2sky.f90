...
!!   - [-inp filename_in]: Name of input wavelet coefficient S2DW formatted 
!!     fits/matlab file.
...
program s2dw_wav2sky
  ...
  case default
     call s2dw_error(S2DW_ERROR_FILEIO, 's2dw_wav2sky', &
  ...
  if(all) then
     ...
     do jj = 0,J
        do gg = 0,N-1
           ...
           call s2_sky_write_map_file(sky, filename_out, &
                comment='Display map computed from S2DW')
     ...
     if(gg<0 .or. gg>=N .or. jj<0 .or. jj>J) then
        call s2dw_error(S2DW_ERROR_ARG_INVALID, 's2dw_wav2sky', &
...
!!! @author J. D. McEwen (mcewen@mrao.cam.ac.uk)
  ...
  subroutine parse_options()
    ...
    do i = 1,n,2
       ...
       case ('-help')
          write(*,'(a)') 'Usage: s2dw_wav2sky [-inp filename_in]'
